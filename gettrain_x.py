...
sample_span={"dieselconl":-30,"dieselconh":30,"stagasdryl":160,"stagasdryh":240,"stagaspresl":50,"stagaspresh":100,"lpgc2l":0,"lpgc2h":6,"lpgc5l":0,"lpgc5h":6}
...
def get_pv(pvfile,curpvs):
	...
	with open(pvfile) as csv_file:
		...
		for row in reader:
			...
			fcc3_pv[tm]=[]
			for pv in curpvs:
				fcc3_pv[tm].append(float(row[pvs.index(pv)+1]))
...
#生产数据文件与化验文件联系到一起
def sample_add_pv(pvfile,samplefile,sample):
	...
	for end_tm,val in samples.items():
		...
		i+=1
	np.savez("sam_pv",na_sam,na_pv)
	

#从npz文件中提取出生产数据及化验数据，按照8：2的比例分成训练集和测试集
def random_and_classes(npzfile):
	r=np.load(npzfile)
#	print(r.files)
	pvs=r["arr_1"]
	sams=r["arr_0"]
	...
	np.savez("train_pvs_sams.npz",train_pvs,train_sams)
	np.savez("test_pvs_sams.npz",test_pvs,test_sams)
...
#timeout()
#gather("produce.csv")
#uniform("produce2.csv","produce3.csv")
